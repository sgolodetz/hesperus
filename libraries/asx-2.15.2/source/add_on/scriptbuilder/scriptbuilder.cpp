// Modified by SMG (31/03/09) in order to get relative #includes right
#define USE_SMG_MODIFICATIONS
...
int CScriptBuilder::ProcessScriptSection(const char *script, const char *sectionname)
{
...
#ifdef USE_SMG_MODIFICATIONS
	{
		...
		if(i != string::npos)
		{
			string originalPath = originalScript.substr(0,i+1);
			for(size_t n=0, size=includes.size(); n<size; ++n)
			{
				includes[n] = originalPath + includes[n];
			}
		}
	}
#endif
