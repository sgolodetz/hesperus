...
public:
	...
	template <typename Poly> static void load_polygons_section(std::istream& is, std::vector<shared_ptr<Poly> >& polygons);
...
public:
	...
	template <typename Poly> static void save_polygons_section(std::ostream& os, const std::vector<shared_ptr<Poly> >& polygons);
