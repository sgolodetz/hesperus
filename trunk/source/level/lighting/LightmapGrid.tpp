...
LightmapGrid::LightmapGrid(const Polygon<Vert,AuxData>& poly, std::vector<TexCoords>& vertexLightmapCoords)
{
	...
	std::vector<Vector3d> projectedVertices = project_vertices_onto(poly, bestAxis);
	make_planar_grid(projectedVertices, vertexLightmapCoords);
...
*/
template <typename Vert, typename AuxData>
std::vector<Vector3d> LightmapGrid::project_vertices_onto(const Polygon<Vert,AuxData>& poly, Axis axis)
{
